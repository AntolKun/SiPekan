...
@section('title', 'Catat Kematian Ikan')
@section('page-title', 'Catat Kematian Ikan')
...
<div class="max-w-3xl mx-auto">
  ...
  <div class="mb-6">
    <div class="flex items-center gap-3 mb-2">
      ...
      <div>
        <h2 class="text-2xl font-bold text-gray-800">Catat Kematian Ikan</h2>
        <p class="text-sm text-gray-600 mt-1">Tambahkan catatan kematian ikan baru</p>
      </div>
    </div>
  </div>
  ...
  <!-- Form -->
  <form action="{{ route('mortality.store') }}" method="POST" class="bg-white rounded-lg shadow-sm p-6">
    @csrf
    ...
    <div class="mb-5">
      ...
      <select name="fish_id" id="fish_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">
    ...
    <div class="mb-5">
      ...
      <select name="reason" id="reason" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 mb-2">
      ...
      <textarea name="reason_custom"
        id="reason_custom"
        rows="3"
        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
        placeholder="Atau tulis penyebab lainnya secara detail...">{{ old('reason_custom') }}</textarea>
    ...
    <div class="mb-5">
      ...
      <input type="datetime-local"
        ...
        id="recorded_date"
        required
        value="{{ old('recorded_date', now()->format('Y-m-d\TH:i')) }}"
        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">
    ...
    <div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
      <div class="flex items-start">
        ...
        <div>
          <p class="text-sm font-medium text-red-800">Estimasi Kerugian</p>
          <p id="lossAmount" class="text-lg font-bold text-red-900 mt-1">Rp 0</p>
          <p class="text-xs text-red-700 mt-1">Dihitung otomatis berdasarkan harga beli × jumlah</p>
        </div>
      </div>
    </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const reasonCustom = document.getElementById('reason_custom');

    // Handle reason selection
    reasonSelect.addEventListener('change', function() {
      if (this.value) {
        reasonCustom.value = this.value;
      }
    });

    reasonCustom.addEventListener('input', function() {
      if (this.value) {
        reasonSelect.value = '';
      }
    });
    ...
    function calculateLoss() {
      ...
      if (!selectedOption.value) {
        lossAmountDisplay.textContent = 'Rp 0';
    ...
    // Initial calculation if old input exists
    if (fishSelect.value) {
      calculateLoss();
    }
  });
